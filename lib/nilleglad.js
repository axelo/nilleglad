...
const weekNo = parseInt(process.argv[2]);
const mayaUsername = process.argv[3];
const mayaPassword = process.argv[4] || '';
...
function parseRaaTimes(html) {
    ...
    const raaRows = rows.filter((i, tr) => $('a', tr).text().startsWith('RAÄ Systemutveckling'));
